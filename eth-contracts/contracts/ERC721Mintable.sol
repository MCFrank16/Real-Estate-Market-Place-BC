...
contract Ownable {
    ...
    event OwnerShipTransefered(address oldOwner, address newOwner);
    ...
    modifier onlyOwner() {
        require(
            msg.sender == _owner,
            "Only Contract owner are allowed to use this contract"
        );
        _;
    }

    // check if the address is a real address
    modifier isAddress(address _address) {
        require(
            Address.isContract(_address) == false,
            "address should be a real address"
        );
        _;
    }
        ...
        isAddress(newOwner)
    {
        ...
        _owner = newOwner;
        emit OwnerShipTransefered(oldOwner, newOwner);
    }

    // 6) get the contract owner
...
contract Pausable is Ownable {
    ...
    function setPaused(bool pause) public onlyOwner {
        require(pause != _paused, "Contract is already paused");
        _paused = pause;
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(
            ...
            "This is already an owner of this token"
        );
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        require(!_exists(tokenId), "The token ID already exists");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
        // require the token exists before setting
        require(_exists(tokenId), "Token provided does not exist");

        // TIP #2: you can also use uint2str() to convert a uint to a string
        string memory _convertToString = uint2str(tokenId);

        // TIP #1: use strConcat() from the imported oraclizeAPI lib to set the complete token URI
        _tokenURIs[tokenId] = strConcat(_baseTokenURI, _convertToString);
    }
}
    ...
    )
{
    ...
    function mint(address to, uint256 tokenId) public returns (bool) {
        require(
            getContractOwner() == msg.sender,
            "Function should only be called by the owner"
        );
        super._mint(to, tokenId);
